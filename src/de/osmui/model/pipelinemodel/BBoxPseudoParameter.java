 ...
 * value setting/getting (overrides this with methods that do nothing/return null) but knows
...
public class BBoxPseudoParameter extends AbstractParameter {
	...
	/* Used to save the Task this param belongs to so we can
	 * manage the parameters making up this pseudo parameter
	 * on our own
	 */
	 ...
	 * Constructor
	 * 
	 * @param desc the description for this bbox pseudo parameter
	 * @param value the value for this pseudo parameter (ignored)
	...
	public BBoxPseudoParameter(TParameter desc, String value, AbstractTask parent, boolean withXY) {
		...
		tParam.setName("left");
		left = new FloatInRangeParameter(tParam, tParam.getDefaultValue() , -180, 180);
		
		tParam = new TParameter();
		tParam.setDefaultValue("180.0");
		tParam.setName("right");
		right = new FloatInRangeParameter(tParam, tParam.getDefaultValue() , -180, 180);
		
		tParam = new TParameter();
		tParam.setDefaultValue("90.0");
		tParam.setName("top");
		top = new FloatInRangeParameter(tParam, tParam.getDefaultValue() , -90, 90);
		
		tParam = new TParameter();
		tParam.setDefaultValue("-90.0");
		tParam.setName("bottom");
		bottom = new FloatInRangeParameter(tParam, tParam.getDefaultValue() , -90, 90);
		...
		if (withXY){
			 ...
			 * osmosis does this will keep it from showing up if it isn't changed
			*/
			tParam = new TParameter();

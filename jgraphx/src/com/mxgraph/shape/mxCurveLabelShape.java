/**
 * $Id: mxCurveLabelShape.java,v 1.16 2010-11-01 22:18:30 david Exp $
...
public class mxCurveLabelShape implements mxITextShape
{
			...
			mxCellState state, Map<String, Object> style)
	{
						...
						rect)))
		{
			...
			for (int j = 0; j < text.length(); j++)
			{
				...
				if (parallel != null && parallel != mxCurve.INVALID_POSITION)
				{
	...
	public mxRectangle updateLabelBounds(String label, Map<String, Object> style)
	{
		...
		for (int j = 0; j < label.length(); j++)
		{
			...
			{
				labelGlyphs[j].labelGlyphBounds.setWidth(0);
				labelGlyphs[j].drawingBounds = INVALID_GLYPH_BOUNDS;
				continue;
			}
			...
			// Bottom left
			double p1X = parallel.getX();
			double minX = p1X, maxX = p1X;
			double p1Y = parallel.getY();
			...
			labelGlyphs[j].drawingBounds = new mxRectangle(minX - 1.0,
					minY - 1.0, maxX - minX + 2.0, maxY - minY + 2.0);

/**
 * $Id: mxStencilShape.java,v 1.8 2010-09-07 20:20:52 david Exp $
 ...
 * and renders it.
 * 
 * See http://projects.gnome.org/dia/custom-shapes for specs.
 * See http://dia-installer.de/shapes_de.html for shapes.
 */
public class mxStencilShape extends mxBasicShape
{
	...
	{
		Document document = mxUtils.parseXml(shapeXml);

		this.name = document.getElementsByTagName("name").item(0)
				.getTextContent();
		this.iconPath = document.getElementsByTagName("icon").item(0)
				.getTextContent();
		this.root = document.getElementsByTagName("svg:svg").item(0);
			...
			double widthRatio, double heightRatio)
	{
		if (shape instanceof Rectangle2D)
		{
			...
			{
				rect.setFrame(rect.getX() + transX, rect.getY() + transY, rect
			...
			if (widthRatio != 1 || heightRatio != 1)
			{
				rect.setFrame(rect.getX() * widthRatio, rect.getY()
						* heightRatio, rect.getWidth() * widthRatio, rect
						.getHeight()
						* heightRatio);
			}
		}
		else if (shape instanceof Line2D)
		{
			...
			if (transX != 0 || transY != 0)
			{
				line.setLine(line.getX1() + transX, line.getY1() + transY, line
						.getX2()
						+ transX, line.getY2() + transY);
			}
			if (widthRatio != 1 || heightRatio != 1)
			{
		...
		else if (shape instanceof Ellipse2D)
		{
			...
			if (widthRatio != 1 || heightRatio != 1)
			{
				ellipse.setFrame(ellipse.getX() * widthRatio, ellipse.getY()
						* heightRatio, ellipse.getWidth() * widthRatio, ellipse
						.getHeight()
						* heightRatio);
			}
		}
	...
	public void createShape(Node root, svgShape shape)
	{
		...
		/*
		 * If root is a group element, then we should add it's styles to the childrens...
		 */
		while (child != null)
		{
	...
	{
		return tag.equals("svg:polyline");
	}
